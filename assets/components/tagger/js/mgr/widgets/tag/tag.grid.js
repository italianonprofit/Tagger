...
Ext.extend(Tagger.grid.Tag,MODx.grid.Grid,{
    ...
    ,assignedChilds: function(btn,e) {
        var assignedChilds = MODx.load({
            xtype: 'tagger-window-assigned-childs'
            ,tagId: this.menu.record.id
            ,title: _('tagger.tag.assigned_childs_to', {tag: this.menu.record.tag})
            ,listeners: {
                'success': {fn:function() { this.refresh(); },scope:this}
            }
        });
    ...
    ,assignChild: function(btn,e) {
        var assignChild = MODx.load({
            ...
            ,tagId: this.menu.record.id
            ,title: 'Assegna un figlio per il tag'
            ,listeners: {
                'success': {fn:function() { this.refresh(); },scope:this}
            }
        });
...
Tagger.grid.AssignedChilds = function(config) {
    config = config || {};
    ...
    Tagger.grid.AssignedChilds.superclass.constructor.call(this,config);

    this.getView().on('refresh', this.refreshSelection, this);
};
Ext.extend(Tagger.grid.AssignedChilds,MODx.grid.Grid,{
    windows: {}
